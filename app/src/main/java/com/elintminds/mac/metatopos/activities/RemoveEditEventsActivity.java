...
public class RemoveEditEventsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    TextView userName, postTime, totalViews, totalComment, totalReviews, postPrice, postTitle, postDescription, postAddress, tv_send_comment, tv_distance, tv_intrested_people;
    ...
    String token, postType;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_remove_edit_events);
    ...
    private void initView() {
        ...
        tv_intrested_people = findViewById(R.id.tv_intrested_people);
...
//            startActivity(i);
            ...
            if (postType.equals("event")) {
                Intent i = new Intent(this, EventDetailFormActivity.class);
                i.putExtra("Edit_Post_Data", postdata.get(0));
                i.putExtra("isFromRemoveEditActivity", "Events");
                startActivity(i);
            }
                ...
                public void onResponse(Call<GetPostByIdResponse> call, Response<GetPostByIdResponse> response) {
                    if (response.isSuccessful()) {
                        if (response.body().getStatus() == true) {
                            ...
                            postType = postdata.get(0).getPostType();
                            imageslider();
                            displayUserData();
...
//                        recyclerView.setAdapter(postCommentAdapter);
    ...
    private void displayUserData() {
        ...
        tv_intrested_people.setText(postdata.get(0).getTotalInterestedPeople());
    ...
    private void imageslider() {
        mPager.setAdapter(new PostImagesSliderAdapter(RemoveEditEventsActivity.this, attachments));
        ...
        if (AppUtils.isInternetIsAvailable(getApplicationContext())) {
            ...
            call.enqueue(new Callback<RemovePostResponse>() {
                ...
                public void onResponse(Call<RemovePostResponse> call, Response<RemovePostResponse> response) {
                    if (response.isSuccessful()) {
                        if (response.body().getStatus() == true) {
                            Toast.makeText(RemoveEditEventsActivity.this, "" + response.body().getMessage(), Toast.LENGTH_SHORT).show();
                            m_Dialog.dismiss();
                            finish();
                        } else {
                            Toast.makeText(getApplicationContext(), "" + response.body().getMessage(), Toast.LENGTH_SHORT).show();
